...
userRouter.get("/", async (req: Request, res: Response) => {
  userModel.findAll((err: Error, users: User[]) => {
    if (err) {
      return res.status(500).json(sendErrorResponse(err));
    }

    res.status(200).json(sendSuccessResponse(users));
  });
});
...
userRouter.post("/", async (req: Request, res: Response) => {
  ...
  userModel.create(newUser, (err: Error, id: number) => {
    ...
    res.status(200).json(sendSuccessResponse(id));
  });
});
...
userRouter.get("/:id", async (req: Request, res: Response) => {
  ...
  userModel.findOne(id, (err: Error, user: User) => {
    ...
    res.status(200).json(sendSuccessResponse(user));
  });
});
